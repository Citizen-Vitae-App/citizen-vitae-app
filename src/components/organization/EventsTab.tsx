...
import { Plus, Search, Calendar as CalendarIcon, Users, Globe, Lock, QrCode, MoreVertical, ChevronUp, ChevronDown, Edit, Trash2, Copy, TrendingUp, Activity, Tag } from 'lucide-react';
...
export function EventsTab({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('event_cause_themes').select(`
          event_id,
  ...
  const kpis = useMemo(() => {
    ...
    if (eventCauseThemes && eventCauseThemes.length > 0) {
      const causeCount = new Map<string, {
        name: string;
        count: number;
        color: string;
      }>();
  ...
  const SortableColumnHeader = ({
    ...
    return <button onClick={() => toggleSort(field)} className="flex items-center gap-1 hover:text-foreground transition-colors">
  ...
  if (isLoading || !organizationId) {
    return <div className="space-y-4 md:space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => <div key={i} className="p-4 rounded-xl border border-border bg-transparent animate-pulse">
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          <div className="p-4 space-y-3">
            {[1, 2, 3, 4, 5].map(i => <div key={i} className="flex items-center gap-4 animate-pulse">
  ...
  return <div className="space-y-4 md:space-y-6">
      ...
      <div className="w-full">
        {filteredEvents.length === 0 ? <div className="border rounded-lg overflow-hidden w-full">
              ...
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="font-semibold w-[14%]">
                    <ColumnHeaderWithFilter label="Date" field="date" filterType="date" icon={<CalendarIcon className="h-4 w-4" />} filters={filters} setFilters={setFilters} sortField={sortField} sortDirection={sortDirection} toggleSort={toggleSort} openFilterPanel={openFilterPanel} setOpenFilterPanel={setOpenFilterPanel} />
                  </TableHead>
                  <TableHead className="font-semibold w-[10%]">
                    <ColumnHeaderWithFilter label="Statut" field="status" filterType="status" filters={filters} setFilters={setFilters} sortField={sortField} sortDirection={sortDirection} toggleSort={toggleSort} openFilterPanel={openFilterPanel} setOpenFilterPanel={setOpenFilterPanel} />
                  </TableHead>
                  <TableHead className="font-semibold w-[10%]">
                    <ColumnHeaderWithFilter label="Visibilité" field="visibility" filterType="visibility" filters={filters} setFilters={setFilters} sortField={sortField} sortDirection={sortDirection} toggleSort={toggleSort} openFilterPanel={openFilterPanel} setOpenFilterPanel={setOpenFilterPanel} />
                  </TableHead>
                  <TableHead className="font-semibold w-[18%]">
                    <SortableColumnHeader label="Lieu" field="location" />
                  </TableHead>
                  <TableHead className="font-semibold w-[10%]">
                    <ColumnHeaderWithFilter label="Part." field="participants" filterType="participants" icon={<Users className="h-4 w-4" />} filters={filters} setFilters={setFilters} sortField={sortField} sortDirection={sortDirection} toggleSort={toggleSort} openFilterPanel={openFilterPanel} setOpenFilterPanel={setOpenFilterPanel} />
                  </TableHead>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                {searchQuery || hasActiveFilters ? 'Aucun événement ne correspond à vos critères' : 'Créez votre premier événement pour commencer'}
              </p>
              {!searchQuery && !hasActiveFilters && !isMember && <Button asChild>
                  <Link to="/organization/create-event">
            ...
            <Table className="table-fixed w-full">
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="sticky top-0 z-10 bg-background font-semibold w-[10%]">
                    <ColumnHeaderWithFilter label="Participants" field="participants" filterType="participants" icon={<Users className="h-4 w-4" />} filters={filters} setFilters={setFilters} sortField={sortField} sortDirection={sortDirection} toggleSort={toggleSort} openFilterPanel={openFilterPanel} setOpenFilterPanel={setOpenFilterPanel} />
                  </TableHead>
                        ...
                        <HoverCard openDelay={200}>
                          <HoverCardTrigger asChild>
                            <div className="space-y-1">
                              ...
                              {capacity && fillRatio !== null && <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                                  <div className={`h-full rounded-full transition-all ${getCapacityProgressColor(participantCount, capacity)}`} style={{
