...
export function EventCalendarView({ events, organizationId, participantCounts, isMember = false, onToolbarReady, onStateChange }: EventCalendarViewProps) {
  ...
  return (
    <div className={currentView === 'dayGridMonth' ? '' : 'overflow-hidden'}>
      {/* FullCalendar */}
      <div className={cn(
        "fc-notion-theme bg-background -mx-4 sm:mx-0 sm:rounded-lg sm:border sm:border-border border-y border-border",
        currentView !== 'dayGridMonth' && "overflow-hidden"
      )}>
        <FullCalendar
          ref={calendarRef}
