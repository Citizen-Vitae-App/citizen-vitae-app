  ...
  return <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            {notifications && notifications.some(n => !n.is_read) && <Button variant="ghost" size="sm" onClick={() => markAllAsRead()} className="text-muted-foreground">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {isLoading ? <div className="space-y-4">
          ...
          </div> : !notifications || notifications.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            {notifications.map(notification => <button key={notification.id} onClick={() => handleNotificationClick(notification)} className={`w-full text-left p-4 rounded-lg border border-border transition-colors ${notification.is_read ? 'bg-background hover:bg-muted/50' : 'bg-muted/30 hover:bg-muted/50'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Action buttons */}
                    {(shouldShowQRCode(notification) || shouldShowCertificate(notification)) && <div className="mt-2">
                        {shouldShowQRCode(notification) && <Button variant="outline" size="sm" className="h-8 text-xs" onClick={e => {
                  e.stopPropagation();
                  ...
                  navigate(`/mes-missions?tab=upcoming&event=${notification.event_id}`);
                        ...
                        {shouldShowCertificate(notification) && <Button variant="outline" size="sm" className="h-8 text-xs" onClick={e => {
                  e.stopPropagation();
